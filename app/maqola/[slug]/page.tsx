    ...
    <main className="min-h-screen bg-cream">
      ...
      <article id="iv-article" className="w-full px-6 lg:px-16 pt-8 pb-20">
        <div className="max-w-[780px] mx-auto flex flex-col gap-6">
          ...
          {/* Everything inside iv-body is what shows in Telegram Instant View */}
  ...
  return (
    <div className="flex items-center gap-4 pt-5">
      ...
      <div className="flex items-center gap-2">
        <ShareBtn href={`https://t.me/share/url?url=${encodeURIComponent(`https://bayanuz.vercel.app/maqola/${article.slug}`)}&text=${encodeURIComponent(article.titleUz)}`} label="Telegram">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
            <path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71L12.6 16.3l-1.99 1.93c-.23.23-.42.42-.83.42z" />
          </svg>
        </ShareBtn>
        <ShareBtn href={`https://wa.me/?text=${encodeURIComponent(article.titleUz + ' — https://bayanuz.vercel.app/maqola/' + article.slug)}`} label="WhatsApp">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
        ...
        <ShareBtn href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.titleUz)}&url=${encodeURIComponent(`https://bayanuz.vercel.app/maqola/${article.slug}`)}`} label="X">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
        </ShareBtn>
        <ShareBtn href={`https://bayanuz.vercel.app/maqola/${article.slug}`} label="Havolani ko'chirish">
